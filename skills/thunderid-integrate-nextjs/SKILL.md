...
WebFetch `https://thunderid.dev/docs/v1.0.x/getting-started/connect-your-application/nextjs.md`

Follow that guide end to end — creating the Console application, installing `@thunderid/nextjs`, adding `ThunderIDProvider` and the proxy/middleware, adding sign-in/sign-up UI, and verifying the app runs — adapting file names, paths, and code style to this project's actual structure rather than copying the doc's file layout verbatim.
...
- **`invalid_client`** — Double-check the Client ID/Client Secret in `.env.local` against the application's General tab.
